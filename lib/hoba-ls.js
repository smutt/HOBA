...
/// this is just obfuscation - anyone who wants can get the
/// JS code and see what to do, but there's probably a slight
/// benefit in this in case some other JS from the same 
/// origin snarfs all of localStorage or in case there's
/// an XSS on the origin site. Note that in those cases this
/// only really helps against an uninterested adversary
/// so obfuscation it is, even if it uses crypto
/// roll on the webcrypto API

function hoba_obsf(val)
{
	var wto=get_tbsorigin();
	var ak="dsafsd"+wto+"sfnhd^&js";
	GibberishAES.size(128);
	var now=new Date();
	var iv=now.getTime()+wto;
	var encrypted = GibberishAES.enc(val, ak, iv);
	return encrypted;
}

function hoba_deobsf(val)
{
	var wto=get_tbsorigin();
	var ak="dsafsd"+wto+"sfnhd^&js";
	GibberishAES.size(128);
	var decrypted = GibberishAES.dec(val, ak);
	return decrypted;
}

var gkname="";
var gknamedone=false;

function hoba_keynam()
{
	if (gknamedone && gkname!="") return gkname;
	var wto=get_tbsorigin();
	var keynam="base64"+wto+"base64isjustalaugh";
	//GibberishAES.size(128);
	//keynam = GibberishAES.enc(keynam, keynam);
	keynam=hex2b64(keynam);
	gkname=keynam;
	gknamedone=true;
	return keynam;
}

function hoba_setpriv(priv)
{
	localStorage.setItem(hoba_keynam(),hoba_obsf(priv));
}

function hoba_getpriv()
{
	var priv;
	var kn=hoba_keynam();
	priv=localStorage.getItem(kn);
	if (priv) return hoba_deobsf(priv);
	// legacy checks, bad b64 padding, then really old
	// unobfuscated keys
	kn+='=';
	priv=localStorage.getItem(kn);
	if (priv) return hoba_deobsf(priv);
	...
	priv=localStorage.getItem(kn);
	if (priv) return hoba_deobsf(priv);
	priv=localStorage.getItem("priv:0");
	if (priv) return priv;
	priv=localStorage.getItem("priv:1");
	if (priv) return priv;
}

function hoba_delpriv()
{
	localStorage.removeItem(hoba_keynam());
	// legacy deletes, just in case
	priv=localStorage.getItem("priv:0");
	priv=localStorage.getItem("priv:1");
}

/// this is an aes impl from 
/// https://raw.github.com/mdp/gibberish-aes/master/src/gibberish-aes.js
