...
function hoba_checkUA2() 
{
	...
	/*
		plan:
			- determine if IE
			- check features that are ok
				- JS, localStorage, worker, 
			- check HOBAstate
			- if no key, generate key and enable register
			- if registered, 
				- enable login/logout as appropriate
				- provide link to account page
	*/
...
module.exports = {
  ...
  get_origin: function get_origin(){
    ...
    // figure out web origin, as scheme:auth:port (no slashes)
    var wo=location;
    var origin = wo.protocol + "://" + wo.hostname + ":" + wo.port;
    var tbsorigin = wo.protocol;
    if (tbsorigin.charAt(tbsorigin.length-1)==":") {
      tbsorigin=tbsorigin.substring(0,tbsorigin.length-1);
    }
    ...
    if (wo.port===null || wo.port==null || wo.port=="") {
      if (wo.protocol=="http:") {
	origin = wo.protocol + wo.hostname + ":80";
	tbsorigin+="80";
      } else if (wo.protocol=="https:") {
	origin = wo.protocol + wo.hostname + ":443";
	tbsorigin+="443";
      } else {
	origin = wo.protocol + wo.hostname + ":0";
      }
    }
    return origin;
  }  
